...
triangles = original_grid.triangles-1
...
for n in [0]: #range(len(fileList)):
   ...
   # print('1st Original depth %.1f \n'%original_grid.coordinates[1511197-1][-1])    
...
#return new gridObj
    ...
    #sub-sampling the DEM data for plot
        ...
        #subplot 312
        #hd0 = axes[1].tripcolor(xy[:,0],xy[:,1],triangles, depth_original, edgecolors = 'none')
        ...
        #hd1 = axes[2].tripcolor(xy[:,0],xy[:,1],triangles, depth_interp, edgecolors = 'none')       
...
#        plt.colorbar(im, cax=cax, **kw)
        picname = os.path.join(dataDir,RasterFileName.split('\\')[-1].split('.')[0:-1][0]+'.png')
